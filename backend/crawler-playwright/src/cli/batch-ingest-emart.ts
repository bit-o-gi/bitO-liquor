...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const keyword of keywords) {
      console.log(`[ingest:emart:batch] crawling keyword: ${keyword}`);
      const { result, html } = await crawlEmartKeyword(page, keyword);

      if (result.debugHtmlSaved) {
        const debugPath = await writeTextArtifact(`debug/ingest-emart-batch-${startedAt}-${slugify(keyword)}.html`, html);
        console.log(`[ingest:emart:batch] debug html saved: ${debugPath}`);
      }
    ...
    const artifactPath = await writeJsonArtifact(`writes/emart-batch-ingest-${startedAt}.json`, {
    ...
    if (options.trace) {
      ...
      const tracePath = `artifacts/traces/emart-batch-ingest-${startedAt}.zip`;
      await context.tracing.stop({ path: tracePath });
      console.log(`[ingest:emart:batch] trace saved: ${tracePath}`);
    }

    console.log(`[ingest:emart:batch] result artifact: ${artifactPath}`);
    console.log(`[ingest:emart:batch] summary json: ${summaryPaths.jsonPath}`);
    console.log(`[ingest:emart:batch] summary md: ${summaryPaths.markdownPath}`);
