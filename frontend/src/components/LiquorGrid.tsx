...
interface LiquorGridProps {
    ...
    loading: boolean;
    error: string | null;
    // 1. onSelect 프롭스 타입 추가
    onSelect: (liquor: GroupedLiquor) => void;
}

// 2. onSelect 프롭스 받기
export default function LiquorGrid({ searchQuery, liquors, loading, error, onSelect }: LiquorGridProps) {
    if (loading) {
        return (
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {liquors.map((l) => (
                    // 3. LiquorCard에 onClick 이벤트로 onSelect 전달
                    <LiquorCard key={l.name} liquor={l} onClick={() => onSelect(l)} />
                ))}
            </div>
        </>
    );
